...
function displayDifferWithDelay(differs: Mino[],callback: ()=>void) {
	...
	clearTimer('fall')
	                    ('fall',displayDifferFallingMinos.bind(null,differsTemp,callback),currentFallingSpeed(currentLevel))
...
let currentMinoLockDownTimer: TimerAbleToEsc = new TimerAbleToEsc();
...
let moveTimers: Map<string, TimerAbleToEsc>;
...
function setTimer(name: string, callback: ()=>void, delay: number): void {
	...
	console.log(new TimerAbleToEsc(callback, delay));
	console.log('$$set timer '+name);
	
	moveTimers.set(name, new TimerAbleToEsc(callback, delay));
...
function countLockDownTimer(): void {
	...
	if (!currentMinoDidLockDown) {
		//clearTimeout(currentMinoLockDownTimer)
		currentMinoLockDownTimer.clearTimeout()
		currentMinoLockDownTimer = new TimerAbleToEsc(lockDown.bind(null,'softDrop'), 500)
